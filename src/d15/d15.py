...
class Day15(Day):
    def solve(self):
        with open(join("src", "d15", "input.txt"), encoding="utf-8") as f:
            ...
            def compaction_pt_1(
                row: int, col: int, dir: str, rows: int, cols: int
            ) -> tuple[int, int]:
            ...
            def compaction_pt_2(
                ...
                def push_row(row: int, cols_prev_row: set[int], dir: str) -> bool:
                    ...
                    for check_col in cols_prev_row:
                        ...
                        if (
                            grid_pt_2[row][check_col] == "["
                            or grid_pt_2[row][check_col] == "]"
                        ):
                ...
                if dir == ">":
                    ...
                    if to_remove > -1:
                        grid_pt_2[row] = (
                            grid_pt_2[row][:col]
                            + ["@"]
                            + grid_pt_2[row][col:to_remove]
                            + grid_pt_2[row][to_remove + 1 :]
                        )
                        grid_pt_2[row][col] = "."
                    ...
                    if to_remove > -1:
                        #
                        grid_pt_2[row] = (
                            grid_pt_2[row][:to_remove]
                            + grid_pt_2[row][to_remove + 1 : col]
                            + ["@"]
                            + grid_pt_2[row][col:]
                        )
                        grid_pt_2[row][col] = "."
            ...
            for move in moves:
                cur_row, cur_col = compaction_pt_1(
                    cur_row, cur_col, move, rows_pt_1, cols_pt_1
                )
                cur_row_pt_2, cur_col_pt_2 = compaction_pt_2(
                    cur_row_pt_2, cur_col_pt_2, move, rows_pt_2, cols_pt_2
                )
