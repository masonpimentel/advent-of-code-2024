...
with open(
    ...
    def explore_region(row: int, col: int, type: str, rows: int, cols: int) -> int:
        ...
        while len(q) > 0:
            ...
            for row_diff, col_diff in [(-1, 0), (0, 1), (1, 0), (0, -1)]:
                ...
                if add_row >= 0 and add_row < rows and add_col >= 0 and add_col < cols:
    ...
    def explore_region_pt_2(row: int, col: int, type: str, rows: int, cols: int) -> int:
        q: list[tuple[int, int]] = deque([(row, col)])
        ...
        while len(q) > 0:
            ...
            up_row = check_row - 1
            no_up_row = up_row < 0
            if (no_up_row or untouched_grid[up_row][check_col] != type) and (check_row, check_col, 'up') not in seen:
                for seen_col in range(check_col - 1, -1, -1):
                    if untouched_grid[check_row][seen_col] == type and (no_up_row or untouched_grid[up_row][seen_col] != type):
            ...
            if (no_right_col or untouched_grid[check_row][right_col] != type) and (check_row, check_col, 'right') not in seen:
                for seen_row in range(check_row - 1, -1, -1):
                    if untouched_grid[seen_row][check_col] == type and (no_right_col or untouched_grid[seen_row][right_col] != type):
            ...
            if (no_down_row or untouched_grid[down_row][check_col] != type) and (check_row, check_col, 'down') not in seen:
                for seen_col in range(check_col - 1, -1, -1):
                    if untouched_grid[check_row][seen_col] == type and (no_down_row or untouched_grid[down_row][seen_col] != type):
            ...
            if (no_left_col or untouched_grid[check_row][left_col] != type) and (check_row, check_col, 'left') not in seen:
                for seen_row in range(check_row - 1, -1, -1):
                    if untouched_grid[seen_row][check_col] == type and (no_left_col or untouched_grid[seen_row][left_col] != type):
            ...
            # print(f'check_row {check_row} check_col {check_col} sides {sides} seen {seen}')
